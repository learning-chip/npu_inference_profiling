...
| **Qwen3.6-35B-A3B W8A8 MoE** Ascend quant | Same probe (`SEQ_LEN=512`, `NUM_GEN=5`, `quantization ascend`) vs Triton | Greedy continuation **matches** Ascend **Triton** vs **PTO staged** vs **PTO mega** (`[279, 15217, 5388, 13, 561]`); first-step vs Triton `max_abs≈1.65` / `≈1.22` (PTO staged / mega), `rmse≈0.47` / `≈0.34` — quantized stack, not tight `numpy.allclose`; **PTO staged vs mega** logits **differ** (`max_abs≈1.28`), unlike dense **27B** — expected under MoE + distinct kernel paths |
