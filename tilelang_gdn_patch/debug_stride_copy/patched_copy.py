...
def _tile_extents_for_buffer_load(
    indices: list, tile_extents: list[tir.PrimExpr]
) -> list[tir.PrimExpr] | None:
    """Map a 2D tile (e.g. ``[C, D]``) onto 4D buffer indices.

    ``copy()`` deduces a 2D tile from the destination buffer. For a 4D global
    tensor, prepending ones only at the **front** assumes the tile occupies the
    **last two** index positions (``[..., t, d]``). That matches
    ``Q[bz, by, i*C, 0]`` (``[B, H, L, D]``) but not ``Q[bz, i*C, by, 0]``
    (``[B, L, H, D]``), where the time chunk and head are at indices 1 and 2.

    Returns:
        A full per-dimension extent list, or ``None`` to use the legacy rule.
    """
    if len(indices) != 4 or len(tile_extents) != 2:
        return None
    e0, e1 = tile_extents[0], tile_extents[1]
    # [B,H,L,D]: L-chunk at index 2 (e.g. i*C), D at index 3
    if isinstance(indices[2], tir.Mul):
        return [1, 1, e0, e1]
    # [B,L,H,D]: L-chunk at index 1, head at index 2, D at index 3
    if isinstance(indices[1], tir.Mul):
        return [1, e0, 1, e1]
    return None
...
def buffer_load_to_tile_region(load: tir.BufferLoad, access_type: str, extents: list[tir.PrimExpr]):
    ...
    if len(indices) > len(extents):
        mapped = _tile_extents_for_buffer_load(indices, extents)
